...
def initializeElements():
    ...
    while ((goldX == 1 and goldY == 1)):
        ...
        if((goldX > 1 and goldY > 1) and (goldX <= grid and goldY <= grid)):
            valid = True
...
def smartScan():
    ...
    global grid
    global gridWin
    global scanCtr, totalCtr, scanRText, rotateText, totalText, rotateCtr, dirText

    for i in range(4):
        #we need to count din later the scan and rotate
        scan_val = scan(grid)
        scanResults.append([scan_val,direction])
        rotate()
...
def smartLevel():
    global smartScanFlag
    global scanResults
    global goldFlag, goldDir
    global beaconFlag, beaconDir
    global filteredBeacon, filteredPit
    global beacon_path1, beacon_path2, beacon_path3
    global moveCtr, totalCtr
    ...
    if smartScanFlag == 0:
        ...
        beaconFlag = 0
        beaconDir = ""
        smartScan() # scanResults contain 4 sides result
        # if there is gold in the current smart scan
        ...
        # if there is/are pit/s in the current smart scan
        for s in scanResults:
            if "P" == s[0]:
                filteredPit.append(s[1])

        # if there is/are beacon/s in the current smart scan
        for s in scanResults:
            if "B" == s[0]:
                beaconDir = s[1]
                beaconFlag = 1
                filteredBeacon.append(s[1])

    # gold found, go directly to gold
    if goldFlag == 1:
        smartScanFlag = 1
        if direction != goldDir:
            return 2
        moveCtr += 1
        totalCtr += 1
        return 3

    # if pit found found in current direction, look away
    if direction in filteredPit:
        smartScanFlag = 1
        return 2

    if beaconFlag == 1:
        if direction != beaconDir:
            smartScanFlag = 1
            return 2
        smartScanFlag = 0
        moveCtr += 1
...
def showGrid():
    ...
    global scanRText, gridWin #check
    global scanText, totalText
    global rotateText, rotateCtr, totalCtr
    global dirRText
    ...
    checker = True
    print("Start Gold Miner --- POSITION: " + str(miner) + " DIRECTION: " + str(direction))
    act = 0
    ...
    while checker:
        ...
        if (act == 1): # Scan
            ...
            print("SCANNED in POSITION: " + str(miner) + " DIRECTION: " + str(direction) + " RESULT: " + result)
            ...
            print("ROTATED in POSITION: " + str(miner) + " DIRECTION: " + str(direction))
            ...
            if miner[0][0] == gold[0] and miner[0][1] == gold[1]:
                gameText.set("Win")
                print("GOLD FOUND in POSITION: " + str(miner))
                checker = False
            ...
            for p in pits:
                if miner[0][0] == p[0] and miner[0][1] == p[1]:
                    gameText.set("Lose")
                    checker = False
                    print("FALLEN INTO PIT in POSITION: " + str(miner))
            ...
            for b in beacons:
                if miner[0][0] == b[0] and miner[0][1] == b[1]:
                    ...
                    beacons.remove([b[0],b[1]])

                    beaconText.set(str(beaconFinal))
                    # value of m; beacon's distance from Gold
                    # return result
                    print("BEACON FOUND in POSITION: " + str(miner) + " BEACON RETURNS: " + str(beaconFinal))
